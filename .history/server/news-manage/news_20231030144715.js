...
router.patch('/update/draft',(req,res)=>{
    ...
    const {title,categoryId,auditState,content,publishTime} = req.body;
    const {id} = req.query;
    
    const sql = "update news set `title`=? , `categoryId`=? , `auditState`=? , `content`=? `publishTime` where `id`=?"
    const arr =[title,categoryId,auditState,content,publishTime,id-0]
    // console.log(arr)
...
router.get('/audit',(req,res)=>{
    // console.log(req.query.auditState);
