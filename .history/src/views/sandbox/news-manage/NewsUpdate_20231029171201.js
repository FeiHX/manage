...
import { Button, Steps, Form, Input, Select, message, notification ,Option} from 'antd'
...
console.log(NewsForm.current)
  useEffect(() => {
    axios.get(`/api/news/preview?id=${props.match.id}`).then(res => {
      console.log(res.data,res.data[0].content)
  ...
  const handleSave = (auditState) => {
    axios.patch(`/api/news/${props.match.id}`, {
      ...formInfo,
      "content": content,
