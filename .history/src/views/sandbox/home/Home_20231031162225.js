...
// console.log(props)
  ...
  const getViewStartData = () => {
    axios.get('/api/news/homepublish?publishState=2').then(res => {
      ...
      console.log(_.groupBy(res.data,item=>item.categoryId),'这里')
      renderBar(_.groupBy(res.data,item=>item.categoryId))
      setAllList(res.data)
...
//有bug有bug有bug有bug有bug有bug有bug有bug有bug有bug有bug有bug有bug有bug有bug有bug有bug有bug有bug
  ...
  const renderPie = () => {
    ...
    for (let i in groupObj) {
      list.push({
        name: props.categories,props.categories.filter(item=>item.id==categoryId)[0].title
...
const mapStateToProps = ({getNews:{news},setCurrentUserReducer:{username,region,role},getCategories:{categories}}) => {
  // console.log(news)
  return {
    news,username,region,role,categories
  }
}
