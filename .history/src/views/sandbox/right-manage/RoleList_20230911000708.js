...
export default function RoleList() {
  ...
  Promise.all([
              ...
              axios.get('/api/rightlistchildren'),
                ...
                console.log(res[1].data.filter(item=>item.rightId ===2))
               let list1= res[0].data.map(item=>
                ({...item,'children':res[1].data.filter(data=>data.rightId === item.id)})
                // {...item,
                // 'children':res[1].data.filter(item=>item.rightId ===2)}
                  // item.children =  res[1].data.filter(item=>item.rightId ===2)
              )
                console.log(list1);
  ...
  const handleOk = ()=>{
    ...
    setdataSource(dataSource.map(item=>{
      if(item.id === currentId) {
        console.log({...item,rcurrentRights)
        return {
          ...item,
