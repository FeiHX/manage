...
import MyTemp from '../right-manage/MyTemp';
const { confirm } = Modal;
...
function UserList(props) {
 ...
 const roleList = props.roles;
  ...
  const handleUpdate = (item) => {
    ...
    setTimeout(()=>{
      console.log(updateForm.current,'limde');
...
console.log(item.id);
    ...
    updateForm.current.validateFields().then(value=>{
      ...
      axios.put(`/api/users?id=${current}`,value).then((res)=>{
        ...
        setdataSource(dataSource.map(item=>{
          if(item.id===current) {
            console.log(current,'///',item,'---',value,roleList);
            return {
              ...item,
              ...value,
  ...
  const memorizedMytemp = useCallback(mytemp,[])
  return (

    <div>
      <MyTemp mytemp={memorizedMytemp}></MyTemp> 
    ...
    >
      ...
      <UserForm ref={addForm} regionList={regionList} roleList={roleList}></UserForm>      </React.Suspense>
    </Modal>
    <Modal
      open={isUpdateOpen}
      title="更新用户"
