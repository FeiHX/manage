  ...
  const renderMenu = menuList => {
    return menuList.map(menuListItem => {
      if (checkPagePermission(menuListItem)) {
        if (
          menuListItem.grade == 1 &&
          menuListItem.rightKey !== "/home" &&
          menuListItem.rightKey !== "/chat"
        ) {
          return (
            menuListItem.children?.length > 0 &&
          ...
          return (
            <Menu.Item key={menuListItem.rightKey}>
              {menuListItem.title}
            </Menu.Item>
          );
        }
