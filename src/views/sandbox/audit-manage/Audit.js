...
function Audit(props) {
  ...
  useEffect(
    () => {
      Axios.get("/api/news/audit?auditState=1").then(res => {
        let list = [];
        res.data?.forEach(item => {
          let role = props.rolelist?.filter(
            data => data.roleType === item.roleId
          );
          item.role = role;
          list.push(item);
        });
        setdataSource(
          roleObj[props.roleId] === "超级管理员"
            ? list
            : [
                ...list.filter(item => item.author === props.username),
                ...list.filter(
                  item =>
                    item.region === props.region &&
                    roleObj[item.roleId] === "区域编辑"
                )
              ]
        );
      });
        ...
        ws.onopen = function() {
          ws.send(
            JSON.stringify({
              ...
              send: props.username,
              recieve: item.author,
              content: `审核通过:用户${item.author}提交的新闻《${item.title}》审核通过`
            })
          );
        };
        ...
        ws.onopen = function() {
          ws.send(
            JSON.stringify({
              ...
              send: props.username,
              recieve: item.author,
              content: `审核未通过:用户${item.author}提交的新闻《${item.title}》被驳回`
            })
          );
        };
      }
...
const mapStateToProps = ({
  CurrentUserReducer: { roleId, region, username },
