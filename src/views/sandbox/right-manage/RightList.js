...
function RightList(props) {
  ...
  const deleteMethod = delItem => {
    if (delItem.grade == 1) {
      setrightListTabData(
        rightListTabData.filter(data => data.id !== delItem.id)
      );
      ...
      (function() {
        let rightListdeletions = rightListTabData
          .filter(data => data.id === delItem.id)[0]
          .children.concat(
            rightListTabData.filter(data => data.id === delItem.id)[0]
          );
        let rolelist = props.rolelist.map(roleListItem => {
          for (let rightListdeletion of rightListdeletions) {
            roleListItem.rights = roleListItem.rights.filter(
              item => item !== rightListdeletion.rightKey
            );
          }
          return roleListItem;
        });
  ...
  const switchMethod = swItem => {
    ...
    if (swItem.grade == 1) {
      swItem.children.forEach(data => {
        data.pagepermission =
          data.pagepermission == 2 ? 2 : swItem.pagepermission;
      });
      setrightListTabData([...rightListTabData]);
      (function() {
        rightlistData.map(data => {
          if (data.id == swItem.id) {
            data = { ...swItem };
          }
        });
        props.changeRightlist(rightlistData);
        Axios.patch(`/api/rightlist?id=${swItem.id}`, {
          pagepermission: swItem.pagepermission
        }).then(res => {
          message.success("权限修改成功");
        });
        Axios.patch(`/api/rightlistchildren/related?rightId=${swItem.id}`, {
          pagepermission: swItem.pagepermission
        });
      })();
      (function() {
        let rightListSwitch = rightListTabData
          .filter(data => data.id == swItem.id)[0]
          .children.concat(
            rightListTabData.filter(data => data.id == swItem.id)[0]
          );
        let rolelist = props.rolelist.map(roleListItem => {
          if (!tag) {
            for (let data of rightListSwitch) {
              roleListItem.rights = roleListItem.rights.filter(
                item => item !== data.rightKey
              );
            }
          } else {
            for (let data of rightListSwitch) {
      ...
      (function() {
        ...
        props.changeRolelist(rolelist);
        Axios.patch(`/api/rolelist/related`, {
          rolelist: rolelist
        }).then(res => {
          message.success("权限修改成功");
        });
      ...
      (function() {
        ...
        props.changeRightlist(rightlistData);
        Axios.patch(`/api/rightlistchildren?id=${swItem.id}`, {
          pagepermission: swItem.pagepermission
        });
      })();
    }
