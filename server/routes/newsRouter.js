...
router.patch("/news/update/draft", (req, res) => {
  ...
  const { id } = req.query;
  const sql =
    "update news set `title`=? , `subheading`=?,`categoryId`=? , `auditState`=? , `content`=?  where `id`=?";
...
router.get("/news/auditlist", (req, res) => {
  const sql =
    "select * from news where `author`= ? AND (`auditState`=1 OR `auditState`=2 OR `auditState`=3) AND (`publishState`=0 OR `publishState`=1)";
...
router.post("/news", (req, res) => {
  ...
  const sql =
    "insert into news (`title`,`subheading`,`categoryId`,`content`,`region`,`author`,`roleId`,`auditState`,`publishState`,`createTime`,`star`,`view`,`publishTime`) values (?,?,?,?,?,?,?,?,?,?,?,?,?)";
  const arr = [
    title,
    subheading,
    categoryId,
